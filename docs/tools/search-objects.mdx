...
For MySQL and MariaDB, `object_type = "schema"` lists every accessible
non-system database and identifies the configured default in `default_schema`.
All other object types stay scoped to the configured default database unless
the caller supplies `schema`. This exposes cross-database discovery without
making ordinary searches fan out across the whole server.
