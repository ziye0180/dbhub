...
When a TOML `execute_sql` tool has `readonly = true`, a user on the DBHub host
can temporarily unlock a single source for data changes without editing the
configuration or restarting the server:

```bash
# Enables the source's configured temporary write capability for 10 minutes
dbhub enable production

# Override the duration (maximum: 1 hour)
dbhub enable production --ttl 30m

# Inspect or close leases
dbhub status
dbhub disable production
```

The CLI writes a host-managed lease to `.dbhub/write-leases.json`. A Docker
deployment should mount `.dbhub` read-only at `/app/.dbhub`, allowing the DBHub
server to verify leases without granting the server permission to create them.
Create the host directory before starting the container, then restrict it to
the host user with mode `0700`.

Without an active lease, a write returns `WRITE_ACCESS_REQUIRED` and tells the
AI to ask the user to run `dbhub enable <source>`. The AI must never execute the
authorization command on the user's behalf.

The command remains identical for both supported modes. With the default
`temporary_write_mode = "dml"`, a lease allows exactly one INSERT, UPDATE, or
DELETE statement per tool call, and UPDATE/DELETE require a top-level WHERE
clause.

An explicitly configured `temporary_write_mode = "migration"` source must be
MySQL/MariaDB and must define a default database. Its lease allows forward
CREATE TABLE, ALTER TABLE ADD, CREATE INDEX, and closed guarded
SET/PREPARE/EXECUTE/DEALLOCATE batches. It rejects ordinary DML, `USE`,
database-qualified write targets, DROP/TRUNCATE, and arbitrary prepared SQL.

`temporary_write_mode = "dml_and_migration"` keeps the same
`dbhub enable <source>` command and issues one lease containing both complete
profiles. Ordinary DML runs in the source default database. Only SQL accepted by
the migration classifier runs in the fixed `temporary_migration_database`; the
target cannot be supplied through MCP input, and the pooled connection is
restored to its default database before reuse.

The issued capability is stored in the lease, so a later configuration change
cannot broaden an already active lease. Expired leases are treated as read-only
without requiring a cleanup command.
