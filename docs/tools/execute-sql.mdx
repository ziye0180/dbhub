...
```bash
# Enables INSERT, UPDATE, and DELETE for 10 minutes
...
A lease allows exactly one `INSERT`, `UPDATE`, or `DELETE` statement per tool
call. `UPDATE` and `DELETE` require a top-level `WHERE` clause. DDL,
administrative SQL, stored routine execution, and multi-statement writes remain
blocked even while a lease is active. Expired leases are treated as read-only
