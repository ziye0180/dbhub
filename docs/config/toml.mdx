...
<ParamField path="temporary_write_mode" type="string" default="dml">
  Select the capability issued by the unchanged host command
  `dbhub enable &lt;source&gt;`. This setting is valid only for `execute_sql` with
  `readonly = true`.

  - `dml`: one INSERT, UPDATE, or DELETE per call; UPDATE and DELETE require a
    top-level WHERE clause.
  - `migration`: MySQL/MariaDB only. The source must define a default database.
    Allows forward CREATE TABLE, ALTER TABLE ADD, CREATE INDEX, and statically
    validated guarded PREPARE sequences in that default database.
  - `dml_and_migration`: MySQL/MariaDB only. Combines the complete DML and
    migration profiles in one lease. DML remains on the source default database;
    migration runs only in `temporary_migration_database`.

  ```toml
  [[tools]]
  name = "execute_sql"
  source = "app_migrations"
  readonly = true
  temporary_write_mode = "migration"
  ```

  Migration mode rejects ordinary DML, `USE`, database-qualified write targets,
  DROP/TRUNCATE, and incomplete or arbitrary prepared statements.
</ParamField>

### temporary_migration_database

<ParamField path="temporary_migration_database" type="string">
  Required when `temporary_write_mode = "dml_and_migration"`. This trusted TOML
  value fixes the database used only for validated migration operations; it is
  never accepted from an MCP argument. The connector restores its source default
  database before returning the pooled connection.
