...
- `dbhub enable <source>` 命令保持单一入口；实际 lease capability 由目标 `execute_sql` 的 `temporary_write_mode` 决定。默认 `dml` 只允许单条受控 INSERT/UPDATE/DELETE；显式 `migration` 只允许 MySQL/MariaDB 默认库中的前向结构迁移；`dml_and_migration` 用同一 lease 同时授予完整 DML profile 和 migration capability。
- hybrid source 的 migration 目标只能由 `temporary_migration_database` 固定配置，不能来自 MCP 参数或 SQL 中的库名前缀。普通 DML 始终使用 source 默认库；migration 执行前在同一连接切到固定目标库，连接归还池前必须恢复默认库，恢复失败则销毁连接。
